...
class ZokoUtils:
    ...
    def create_product_detail_message(self, product_data: Dict) -> Dict:
        ...
        template_args = [
            product_data.get("image_url", "https://via.placeholder.com/400x300?text=Product"),
            product_data.get("title", "Product"),
            f"PROD-{product_data.get('id', 'unknown')}",
            f"buy_now_{product_data.get('id', 'unknown')}"
        ]
        return {
            "whatsapp_type": "buttonTemplate",
            "template": {
