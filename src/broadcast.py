...
# Load message templates from templates.json, fallback to default if error
# occurs
try:
    ...
    if not isinstance(TEMPLATES.get("promo"), dict):
        raise ValueError(
            "Invalid templates.json: 'promo' must be a dictionary")
except Exception as e:
    logger.error(f"Failed to load templates.json: {str(e)}")
    # Fallback to a default templa te if loading fails
...
def broadcast_promo():
    ...
    for u in users:
        ...
        # Select the template for the user's language, fallback to English if
        # not found
        text = TEMPLATES["promo"].get(
            lang, TEMPLATES["promo"]["en"]).format(
            name=name)
        try:
