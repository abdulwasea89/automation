...
class ChatMemory:
    ...
    def save_message(chat_id: str, role: str, msg: str) -> None:
        ...
        if role not in ("user", "bot"):
            logger.warning(
                f"Unknown role '{role}' for {chat_id}, saving as 'user'")
            role = "user"
        ...
        try:
            ...
            ref.update(
                {ChatMemory.MESSAGES_FIELD: firestore.ArrayUnion([record])})
    ...
    def load_history(chat_id: str) -> List[Dict]:
        ...
        try:
            ...
            logger.info(
                f"Loaded {len(valid_messages)} valid messages for {chat_id}")
            if len(valid_messages) < len(messages):
                logger.warning(
                    f"Filtered out {len(messages) - len(valid_messages)} invalid messages for {chat_id}")
